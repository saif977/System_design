...
public:
    ...
    SandwichBuilder &addDefaultSide()
    {
        ...
        for (Side side : s)
        {
            cout << "Adding Side : " << sideToString(side) << endl;
...
// Director instructs the way of constructing or building things
// for ex , The building of sandwiches differs for DineInOrder,TakeAwayOrder,AddWrapper,OrderWithNoSide,AddWithoutSauce etc.
//=========== Director Class ============================================
class Director{
    public:
     Sandwich buildSandwich_DineIn(SandwichBuilder* sandwich)
     {
        cout<<"Building Sandwich with Dine In"<<endl;
        // No wrapper needed Since dinein.
        (*sandwich).addBread().addProtein().addDefaultSide().addDefaultSauce();
        return (*sandwich).getSandwich();
     }
     Sandwich buildSandwich_TakeAway(SandwichBuilder* sandwich)
     {
        cout<<"Building Sandwich with Take Away"<<endl;
        // Add wrapper Since take away.
        (*sandwich).addBread().addProtein().addDefaultSide().addDefaultSauce().addWrapper();
        return (*sandwich).getSandwich();
     }
     Sandwich buildSandwich_NoSide(SandwichBuilder* sandwich)
     {
         cout<<"Building Sandwich with No Side"<<endl;
        // No side, Since Order with no side.
        (*sandwich).addBread().addProtein().addDefaultSauce().addWrapper();
        return (*sandwich).getSandwich();
     }
     Sandwich buildSandwich_NoSauce(SandwichBuilder* sandwich)
     {
         cout<<"Building Sandwich with No Sauce"<<endl;
        // No sauce, Since Order with no sauce.
        (*sandwich).addBread().addProtein().addDefaultSide().addWrapper();
        return (*sandwich).getSandwich();
     }
};
//=======================================================================
...
int main()
{
    ...
    //========= Building different burgers with director ==========
    Director *d = new Director();
    Sandwich vegBurgerSandwich_dineIn = d->buildSandwich_DineIn(new VegBurgerSandwich());
    Sandwich chickenHotDogSandwich_takeaway = d->buildSandwich_TakeAway(new HotDogSandwich());
    Sandwich chickenHotDogSandwich_NoSide = d->buildSandwich_NoSide(new HotDogSandwich());
    Sandwich vegBurgerSandwich_NoSauce = d->buildSandwich_NoSauce(new HotDogSandwich());
    //========================================================
